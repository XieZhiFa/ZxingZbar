...
public final class PreferencesFragment
    ...
    PreferenceCategory preferences_scanning_category;
    ListPreference preferences_decoder;
    ...
    public void onCreate(Bundle icicle) {
        ...
        preferences_scanning_category = (PreferenceCategory) findPreference(PreferencesActivity.KEY_SCANNING_CATEGORY);
        preferences_decoder = (ListPreference) findPreference(PreferencesActivity.KEY_DECODER_TYPE);
        setScanningSummary(DecoderMode.readPref(preferences.getSharedPreferences()));
        preferences_decoder.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                setScanningSummary(DecoderMode.parse(newValue.toString()));
                return true;
            }
        });
    }
    ...
    private void setScanningSummary(DecoderMode mode) {
        if (mode == DecoderMode.Zxing) { //如果是Zxing
            preferences_decoder.setSummary(getString(R.string.preferences_decoder_summary_zxing));
            getPreferenceScreen().addPreference(preferences_scanning_category);
        } else {
            preferences_decoder.setSummary(getString(R.string.preferences_decoder_summary_zbar));
            getPreferenceScreen().removePreference(preferences_scanning_category);
        }
    }
